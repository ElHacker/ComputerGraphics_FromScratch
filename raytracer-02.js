...
let ComputeLighting = (point, normal) => {
  let intensity = 0;
  let length_n = Length(normal); // Since this is a normal vector, the length should be 1.0 always.
  for (let i = 0; i < lights.length; i++) {
    ...
    if (light.type === Light.AMBIENT) {
      ...
      if (light.type === Light.POINT) {
        vec_1 = Subtract(light.position, point);
      } else if (light.type === Light.DIRECTIONAL) {
        vec_1 = light.position;
      }
      const n_dot_1 = DotProduct(normal, vec_1);
      if (n_dot_1 > 0) {
        intensity += light.intensity * n_dot_1 / (length_n * Length(vec_1));
      }
    }
  }
  return intensity;
